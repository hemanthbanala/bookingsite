...
const Home = () => {
  ...
  const fetchMovieDetails = async (movieId) => {
    ...
    const cast = creditsResponse.data.cast.slice(0, 5); // Limit to top 5 cast members
    setDetailMovie({
      ...movieResponse.data,
      director,
      cast,
  ...
  return (
    <div className="p-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {filtered.map(movie => (
          <MovieCard
            ...
            onBook={m => { setSelectedMovie(m); setModalOpen(true); }}
            onPlayTrailer={m => { setTrailerMovie(m); setTrailerOpen(true); }}
          />
        ))}
      </div>
      ...
      <BookingModal open={modalOpen} onClose={() => setModalOpen(false)} movie={selectedMovie || {}} onBook={() => setModalOpen(false)} />
      <TrailerModal open={trailerOpen} onClose={() => setTrailerOpen(false)} movie={trailerMovie} />
      {detailOpen && detailMovie && (
        <div
          className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50"
          onClick={() => setDetailOpen(false)} // Close popup when clicking outside
        >
          <div
            className="bg-white p-8 rounded-2xl shadow-2xl w-96 relative"
            onClick={(e) => e.stopPropagation()} // Prevent closing when clicking inside
          >
            <button onClick={() => setDetailOpen(false)} className="absolute top-3 right-3 text-gray-400 hover:text-gray-700 text-xl font-bold">&times;</button>
            <img
              src={`https://image.tmdb.org/t/p/w500${detailMovie.poster_path}`}
              alt={detailMovie.title}
              className="rounded shadow-lg w-full h-64 object-cover mb-4" // Adjust image to be square and comfortable in size
            />
            <h2 className="text-2xl font-bold mb-2">{detailMovie.title}</h2>
            <p className="text-lg mb-4">Rating: {detailMovie.vote_average}/10</p>
