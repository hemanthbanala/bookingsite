...
const MovieCard = ({ movie, genres = [], onBook, onImageClick, onPlayTrailer }) => {
  ...
  return (
    <div className="bg-white rounded shadow p-4 flex flex-col group relative">
      <div className="relative">
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="currentColor" viewBox="0 0 20 20"><path d="M6.5 5.5v9l8-4.5-8-4.5z" /></svg>
        </button>
      </div>
      <div className="flex items-center gap-1 mb-1">
        <StarIcon fontSize="small" className="text-yellow-500" />
        <span className="font-semibold text-sm text-gray-700">{movie.vote_average?.toFixed(1)} / 10</span>
      </div>
      <h3 className="font-bold text-lg mb-1">{movie.title}</h3>
      <p className="text-sm mb-2">{movie.release_date}</p>
      {genreNames && (
        <p className="text-xs text-gray-500 mb-1">{genreNames}</p>
      )}
      {movie.original_language && (
        <p className="text-xs text-gray-600 mb-2">Language: <span className="font-semibold">{languageName}</span></p>
      )}
      <button onClick={() => onBook(movie)} className="bg-green-500 text-white px-2 py-1 rounded mt-auto">Book</button>
    </div>
  );
};
